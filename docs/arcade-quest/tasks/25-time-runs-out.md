# Time Runs Out { .arcade-task }

## Goal

Make the game end when the timer reaches zero.

## Do This

1. In the sprite list below the Stage, click the `Player` sprite, or click the Stage thumbnail. (If you cannot remember where the sprite list is, go back to [Scratch Help: Where To Click Sprites](../scratch-help.md#where-to-click-sprites).)
2. Use the `time = 0` part of the code from Count Down.
3. Make it broadcast `game over`.
4. Test by changing the starting time to `5`.
5. Change the starting time back to `60` when it works.

## Check

When `time` reaches `0`, the game should send `game over`.
