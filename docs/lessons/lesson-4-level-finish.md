...
2. Create a variable called `time`.
...
## Code Image: Timer And Finish Flag
...
## Optional Code Image: Side-Scrolling Camera

This script keeps the player near the middle of the screen and changes a `scroll x` variable. Use it only if you are ready for a harder challenge. Sprites in the level can use `set x to ((start x) + (scroll x))` to move with the camera.
